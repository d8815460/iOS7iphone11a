...
- (void)setFrameRate:(NSInteger)frameRate;
{
	...
	if (_frameRate > 0)
	{
		if ([_inputCamera respondsToSelector:@selector(setActiveVideoMinFrameDuration:)] &&
            ...
            if (error == nil) {
                [_inputCamera setActiveVideoMinFrameDuration:CMTimeMake(1, (int)_frameRate)];
                [_inputCamera setActiveVideoMaxFrameDuration:CMTimeMake(1, (int)_frameRate)];
            }
...
- (void)processVideoSampleBuffer:(CMSampleBufferRef)sampleBuffer;
{
    ...
    else
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, bytesPerRow / 4.0f, bufferHeight, 0, GL_BGRA, GL_UNSIGNED_BYTE, CVPixelBufferGetBaseAddress(cameraFrame));
